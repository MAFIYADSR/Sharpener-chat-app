...
exports.getMessages = async (req, res) => {
    try {
        ...
        if (!lastMessageId || lastMessageId == -1) {    //I have to work here
            ...
            // const arrayAsJSON = req.query.array;
            // const receivedArray = JSON.parse(decodeURIComponent(arrayAsJSON));
            // console.log(receivedArray);
            // messages = await sequelize.query(`SELECT * FROM chathistories WHERE id > ${3}`);
            messages = await ChatHistory.findAll({
                attributes: ['id', 'message', 'isImage', 'groupId'],
                where: {
                    groupId: Number(groupId),
        ...
        // const query = `SELECT * FROM ${ChatHistory} WHERE ${ChatHistory.id} > ${lastMessageId};`

        // $query = "SELECT * FROM `ChatHistory` WHERE `ChatHistory.id` > '$lastMessageId' ORDER BY `createdAt` DESC";

        // let messages = "SELECT * FROM `ChatHistory` WHERE `ChatHistory.id` > 'lastMessageId' "
